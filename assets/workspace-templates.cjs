...
const WORKSPACE_TEMPLATES = [
  ...
  // .cursor rules + agents
  {
    ...
    content: T([
      ...
      "",
      "## Invoking subagents (Cursor behavior)",
      "Depending on the Cursor build/plan, subagents may be invoked via:",
      "- **Automatic delegation** (Agent chooses the right subagent based on `description` fields), and/or",
      "- **Explicit invocation** via slash commands like:",
      "  - `/goalguard-worker ...`",
      "  - `/goalguard-verifier ...`",
      "  - `/goalguard-repo-searcher ...`",
      "",
      "If subagents are not usable, use the single-chat fallback below.",
      "",
  ...
  // Slash command helper
  {
    ...
    content: T([
      ...
      "4) Use the Worker subagent (`goalguard-worker`) for the next checkpoint",
      "   - If automatic delegation doesn't work, explicitly invoke it via `/goalguard-worker ...`",
      "5) Verify using the Verifier subagent (`goalguard-verifier`)",
      "   - If needed, explicitly invoke it via `/goalguard-verifier ...`",
